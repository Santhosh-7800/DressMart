...
import { useAllAdminProducts, useSetProductStatus, useSetProductFeatured, useSetProductDealOfDay, useDeleteProduct } from '@/hooks/useAdminProducts';
...
function ProductRowActions({ product, onToggleHidden, onDelete }: { product: Product; onToggleHidden: () => void; onDelete: () => void }) {
  return (
    <div className="flex items-center justify-end gap-1">
      <Link to={`/admin/products/${product.id}/edit`} className="tap-target-48 rounded-lg p-1.5 hover:bg-primary-100 dark:hover:bg-primary-700" title="Edit" aria-label="Edit product">
        <Pencil size={15} />
      </Link>
 ...
 */
export function AdminAllProductsPage() {
  ...
  const { data: allProducts = [], isLoading } = useAllAdminProducts();
