...
/**
 * Desktop/tablet-only rich Profile dashboard — split out of ProfilePage so its data hooks
 * (orders/wishlist/recently-viewed/addresses/coupons/recommendations) only ever run when this
 * component is actually mounted. ProfilePage previously rendered this same content unconditionally
 * with `hidden md:block`/`hidden md:grid` — pure CSS hiding, so every one of these queries and the
 * wishlist/recently-viewed/recommended ProductCard grids still fired and rendered invisibly on
 * mobile. ProfilePage now only mounts this component at all when `useMediaQuery('(min-width: 768px)')`
 * is true, so mobile gets the minimal list (ProfileMobileList) with none of this data fetched.
 */
export function ProfileDesktopDashboard({ user, onSignOut }: ProfileDesktopDashboardProps) {
  ...
  const handleSave = async () => {
    try {
      ...
      toast.error(getFriendlyErrorMessage(error, 'Update failed'));
    }
  };
    ...
    <div className="space-y-6">
      ...
      <Card className="p-2">
        {MORE_LINKS.map(({ to, label, icon: Icon }) => (
          <Link key={to} to={to} className="flex items-center gap-3 rounded-2xl px-3 py-3 transition-colors hover:bg-acc-primary/5">
