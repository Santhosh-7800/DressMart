...
import type { Gender, ProductStatus, AdminProductColorInput, AdminProductInput } from '@/types';
...
function ColorCard({
  ...
  /** Staff without the `upload_images` permission can still see this color's existing photos but
   *  can't add/remove/reorder them — always true for the Admin (see the call site). */
  canUploadImages: boolean;
  /** Staff without `manage_inventory` can see stock counts but not change them here. */
  ...
  const handleFiles = async (files: FileList | File[]) => {
    ...
    // Downscale to a sane max width before it ever sits in memory as a preview/upload candidate —
    // a phone camera photo can be 4-8MB; nothing about a product thumbnail needs that. Falls back
    // to the original file if compression fails, rather than silently dropping the photo.
    const items: ColorImageItem[] = await Promise.all(
      toAdd.map(async (file) => {
        try {
          ...
          return { id: newId(), url: URL.createObjectURL(compressedFile), file: compressedFile };
        } catch {
          return { id: newId(), url: URL.createObjectURL(file), file };
        }
      }),
    );
    ...
    <div className="rounded-2xl border border-primary-200 p-4 dark:border-primary-700">
      ...
      {canUploadImages ? (
        ...
        <p className="rounded-xl border border-dashed border-primary-200 p-4 text-center text-xs text-primary-400 dark:border-primary-600">
          You don't have permission to upload images. Ask the shop owner to grant "Upload Product Images" access.
        </p>
      )}
            ...
            >
              ...
              {canUploadImages && (
                <button
                  ...
                  className="tap-target-48 absolute -right-1.5 -top-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-primary-900 text-white shadow-sm transition-transform duration-200 hover:scale-110"
            ...
            return (
              ...
              >
                <label className="flex items-center gap-1.5 font-medium">
                  <input type="checkbox" checked={checked} disabled={!canManageInventory} onChange={() => toggleSize(size)} className="h-3.5 w-3.5 rounded disabled:opacity-40" />
                  {size}
                </label>
                <input
                  type="number"
                  min={0}
                  disabled={!checked || !canManageInventory}
...
export function AdminProductFormPage() {
  ...
  // Field-level gates within an otherwise-permitted form — a staff member who can add/edit
  // products at all doesn't automatically get every sub-capability; upload_images/manage_inventory
  // are granted (or not) independently. Always true for the Admin (no such restriction).
  const canUploadImages = !isStaff || Boolean(permissions?.upload_images);
  const canManageInventory = !isStaff || Boolean(permissions?.manage_inventory);
  ...
  const handleSubmit = async (overrideStatus?: ProductStatus) => {
    ...
    if (!user) return;
    // Firestore's offline persistence queues writes made while offline and their promises don't
    // resolve/reject until connectivity returns — without this check, the Save button would just
    // spin indefinitely instead of clearly telling the user nothing was saved yet.
    if (!isOnline) {
      toast.error("You're offline. Reconnect and try saving again.");
      return;
    }
    ...
    let colors: AdminProductColorInput[];
    try {
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <div className="space-y-6">
        ...
        {/* Pricing — owner-only. Staff can VIEW price/MRP/GST (per the "viewing product
            information" responsibility) but never set/change them: pricing is a financial
            decision, and per the business model only the Head Seller/seller owns financial terms.
            Enforced server-side too (firestore.rules' products create/update rules), not just by
