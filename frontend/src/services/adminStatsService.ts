...
/** Admin-only platform-wide summary — shown as an extra section on the same dashboard. */
 ...
 *  recent-orders slice would have for the Admin viewing platform-wide counts. */
...
export const adminStatsService = {
  async getSellerOverview(sellerId: string): Promise<SellerOverviewStats> {
    ...
    const [
      ...
      // can't express as a query constraint — fetched and filtered client-side instead. The same
      // snapshot also gives out-of-stock count and total units for free (see below) — Firestore has
      // no boolean/derived-field index to aggregate either of those server-side.
      getDocs(query(inventoryCol, where('seller_id', '==', sellerId))),
      // Full product docs (not just a count) — needed for totalVariants/discontinuedProducts, which
      // have no server-side aggregate equivalent either. Bounded to this seller's own catalog, same
      // read this file's listLowStock/listOutOfStock already do.
    ...
    const inventoryDocs = inventorySnap.docs.map((d) => d.data() as { total_stock: number; low_stock_threshold: number });
    // "Low stock" here means "at or below threshold, including zero" to match every other low-stock
    // read in this codebase (listLowStock/SellerInventoryPage) EXCEPT those explicitly exclude zero
    // (0 < total_stock <= threshold) so a product isn't double-counted as both low AND out. Mirror
    // that here: outOfStock is total_stock === 0; lowStockCount excludes those.
    const outOfStockProducts = inventoryDocs.filter((d) => d.total_stock <= 0).length;
    const lowStockCount = inventoryDocs.filter((d) => d.total_stock > 0 && d.total_stock <= d.low_stock_threshold).length;
    const totalUnits = inventoryDocs.reduce((sum, d) => sum + Math.max(0, d.total_stock), 0);
    const totalVariants = products.reduce((sum, p) => sum + p.variants.length, 0);
    // DISCONTINUED reuses the existing status === 'hidden' value (Phase 12 audit: ProductStatus
    // already distinguishes 'hidden' — a seller's deliberate "pull this from the store" action —
    // from 'draft' (not yet published) and 'out_of_stock' (still a real, catalog-visible listing
    // per isActiveStatus); 'hidden' is the one that matches "discontinued," so no new field was added.
    const discontinuedProducts = products.filter((p) => p.status === 'hidden').length;
  ...
  async getPlatformOverview(): Promise<PlatformOverviewStats> {
    ...
    const [ordersAgg, revenueAgg, customersAgg, staffAgg, activeCoupons, todayStats, monthStats] = await Promise.all([
