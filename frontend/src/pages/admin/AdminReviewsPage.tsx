...
import { MessageSquare, BadgeCheck, EyeOff, Eye } from 'lucide-react';
...
export function AdminReviewsPage() {
  ...
  const hideMutation = useMutation({
    mutationFn: ({ reviewId, isHidden }: { reviewId: string; isHidden: boolean }) => reviewService.setHidden(reviewId, isHidden),
    onSuccess: (_data, { reviewId, isHidden }) => {
      toast.success(isHidden ? 'Review hidden' : 'Review restored');
      queryClient.invalidateQueries({ queryKey: ['reviews', 'seller', user?.id ?? ''] });
      const review = (reviewsQuery.data ?? []).find((r) => r.id === reviewId);
      if (review) queryClient.invalidateQueries({ queryKey: queryKeys.reviews.byProduct(review.product_id) });
    },
    onError: (error: Error) => toast.error(error.message || 'Could not update this review.'),
  });
      ...
      {reviewsQuery.isLoading ? (
        ...
        <div className="space-y-3">
          {reviews.map((review) => {
            ...
            return (
              <Card key={review.id} hover={false} className={review.is_hidden ? 'opacity-60' : undefined}>
                <div className="mb-2 flex items-center justify-between gap-3">
                  <div>
                    ...
                    <div className="mt-1 flex items-center gap-2">
                      ...
                      {review.is_hidden && <span className="rounded-full bg-primary-200 px-2 py-0.5 text-xs font-semibold text-primary-600 dark:bg-primary-700 dark:text-primary-200">Hidden</span>}
                    </div>
                  </div>
                  <div className="flex shrink-0 items-center gap-2">
                    <p className="text-xs text-acc-text-secondary">{formatDate(review.created_at)}</p>
                    <button
                      title={review.is_hidden ? 'Restore review' : 'Hide review'}
                      onClick={() => hideMutation.mutate({ reviewId: review.id, isHidden: !review.is_hidden })}
                      className="rounded-full p-1.5 text-acc-text-secondary hover:bg-primary-100 hover:text-acc-primary dark:hover:bg-primary-800"
                    >
