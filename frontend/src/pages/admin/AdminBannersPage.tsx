...
const EMPTY_FORM: BannerFormState = { image_url: '', title: '', subtitle: '', cta_label: 'Shop Now', link: '/', start_at: '', end_at: '', sort_order: 0, is_active: true };
...
function bannerToForm(banner: Banner): BannerFormState {
  return {
    ...
    link: banner.link,
    start_at: banner.start_at ? banner.start_at.slice(0, 10) : '',
    end_at: banner.end_at ? banner.end_at.slice(0, 10) : '',
    ...
    <div className="space-y-6">
              ...
              <div className="flex min-w-0 items-center gap-3">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {banner.is_active && banner.start_at && new Date(banner.start_at) > new Date() && (
                      <span className="rounded-full bg-blue-50 px-2.5 py-0.5 text-xs font-semibold text-blue-700 dark:bg-blue-900/30 dark:text-blue-300">Scheduled</span>
                    )}
                    {banner.is_active && banner.end_at && new Date(banner.end_at) < new Date() && (
                      <span className="rounded-full bg-red-50 px-2.5 py-0.5 text-xs font-semibold text-red-600 dark:bg-red-900/30 dark:text-red-300">Expired</span>
                    )}
                  </div>
                  ...
                  <p className="text-xs text-acc-text-secondary">
                    ...
                    {(banner.start_at || banner.end_at) &&
                      ` · ${banner.start_at ? formatDate(banner.start_at) : 'Anytime'} – ${banner.end_at ? formatDate(banner.end_at) : 'No end'}`}
                  </p>
                </div>
              </div>
      ...
      <Modal isOpen={isFormOpen} onClose={() => setIsFormOpen(false)} title={editingBanner ? 'Edit Banner' : 'New Banner'}>
        <div className="space-y-4">
          ...
          <div className="flex gap-3">
            <Input floating label="Starts (optional)" type="date" value={form.start_at} onChange={(e) => setForm({ ...form, start_at: e.target.value })} />
            <Input floating label="Ends (optional)" type="date" value={form.end_at} onChange={(e) => setForm({ ...form, end_at: e.target.value })} />
          </div>
          <div className="flex gap-3">
