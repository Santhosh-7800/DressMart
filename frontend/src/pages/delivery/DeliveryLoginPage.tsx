...
/** Mirrors StaffLoginPage's password flow (delivery accounts are created email/password-only via
 *  the Head Seller's Add Delivery Person form — no phone/Google sign-in path for delivery). */
export function DeliveryLoginPage() {
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      if (!profile || !isDeliveryRole(profile.role)) {
        await authService.signOut();
        toast.error('This account is not registered as Delivery Staff.');
        return;
      }
  ...
  return (
    <div>
      <Seo title="Delivery Login" />
      <h1 className="text-2xl font-bold">Delivery Login</h1>
      <p className="mt-1 text-sm text-primary-400">Sign in to your DressMart delivery account</p>
