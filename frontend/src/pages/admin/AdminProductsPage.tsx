...
import { useAdminProducts, useSetProductStatus, useDeleteProduct, useDuplicateProduct, useSetProductDealOfDay } from '@/hooks/useAdminProducts';
...
export function AdminProductsPage() {
  ...
  // Permission gate only applies to staff — the Admin keeps full access, same as today.
  ...
  // Deal-of-Day ("Offers") isn't part of the staff permission set — Admin only.
  ...
  const { data: allProducts = [], isLoading: isLoadingProducts, isError: isProductsError, refetch: refetchProducts } = useAdminProducts();
    ...
    <div>
      ...
      {isLoading ? (
        ...
        <Card hover={false} className="flex flex-col items-center gap-3 py-8 text-center">
          ...
          <p className="text-sm text-primary-500">Couldn't load products.</p>
          <Button variant="outline" size="sm" onClick={() => refetchProducts()}>
            Retry
          </Button>
        </Card>
      ) : allProducts.length === 0 ? (
        <EmptyState icon={PackageSearch} title="Nothing to show yet" description="Add your first product to get started." />
      ) : (
        <>
